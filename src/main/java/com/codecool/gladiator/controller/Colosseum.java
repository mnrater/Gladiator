...
public class Colosseum {
    ...
    private List<Contestants> splitGladiatorsIntoPairs(List<Gladiator> gladiators) {
        // Todo
        List<Contestants> contestans = new LinkedList<>();
        for (int i = 0; i < gladiators.size()/2; i++) {
            contestans.add(new Contestants(
                    gladiators.get(i),
                    gladiators.get(i+gladiators.size()/2)));
        }
        return contestans;
    }
    ...
    private Gladiator getChampion(Tournament tournament) {
        ...
        // reverse tree
        // >> in leaf
        // > left branch fight and  winner go up
        // > right branch fight and go up
        // > repeat
        if (tournament.getContestants() == null) {
            Gladiator winner1 = getChampion(tournament.getLeftBranch());
            Gladiator winner2 = getChampion(tournament.getRightBranch());
            tournament.setContestants(new Contestants(winner1, winner2));
        }
    ...
    private Gladiator simulateCombat(Combat combat) {
        ...
        announceWinnerAndLoser(combat.getWinner(), combat.getLoser());
